import type { KeyboardEvent } from "react";
import { motion } from "framer-motion";
import WashiTape from "../../components/WashiTape";
import type { ProjectType } from "./ProjectsData";

type ProjectPolaroidProps = {
  title: string;
  tagline: string;
  thumbnail: string;
  githubUrl: string;
  webUrl?: string;
  type: ProjectType;
  onClick: () => void;
};

function ProjectPolaroid({
  title,
  tagline,
  thumbnail,
  githubUrl,
  webUrl,
  type,
  onClick,
}: ProjectPolaroidProps) {
  const handleKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      onClick();
    }
  };

  return (
    <motion.div
      role="button"
      tabIndex={0}
      onClick={onClick}
      onKeyDown={handleKeyDown}
      whileHover={{ scale: 1.06, y: -10, zIndex: 30 }}
      whileTap={{ scale: 0.97 }}
      transition={{ type: "spring", stiffness: 260, damping: 18 }}
      className="relative z-10 mx-auto w-full max-w-sm cursor-pointer rounded-md bg-white p-4 pb-10 text-left shadow-lg outline-none hover:shadow-2xl"
    >
      <WashiTape color="blue" rotate={0} className="absolute -top-6 left-1/2 -translate-x-1/2" />

      <img
        src={thumbnail}
        alt={title}
        className="aspect-square w-full rounded-sm object-cover"
      />

      <h3 className="mt-3 font-jua text-xl text-[#000097]">{title}</h3>
      <p className="mt-1 text-base leading-snug text-[#000097]/70">{tagline}</p>

      <div className="mt-2 flex items-center justify-between">
        <span
          className={`inline-block rounded-full px-3 py-1 text-xs font-semibold ${
            type === "solo"
              ? "bg-green-100 text-green-700"
              : "bg-[#000097] text-white"
          }`}
        >
          {type === "solo" ? "Solo Project" : "Team Project"}
        </span>

        <div className="flex items-center gap-3">
          {webUrl && (
            <a
              href={webUrl}
              target="_blank"
              rel="noopener noreferrer"
              onClick={(event) => event.stopPropagation()}
              aria-label={`Visit ${title} site`}
              className="text-[#000097]/70 transition-colors hover:text-[#000097]"
            >
              <ExternalLinkIcon className="h-5 w-5" />
            </a>
          )}

          {githubUrl ? (
            <a
              href={githubUrl}
              target="_blank"
              rel="noopener noreferrer"
              onClick={(event) => event.stopPropagation()}
              aria-label={`View ${title} on GitHub`}
              className="text-[#000097]/70 transition-colors hover:text-[#000097]"
            >
              <GithubIcon className="h-5 w-5" />
            </a>
          ) : (
            <span
              aria-hidden="true"
              className="text-[#000097]/25"
              title="GitHub link coming soon"
            >
              <GithubIcon className="h-5 w-5" />
            </span>
          )}
        </div>
      </div>
    </motion.div>
  );
}

type ExternalLinkIconProps = {
  className?: string;
};

function ExternalLinkIcon({ className }: ExternalLinkIconProps) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      className={className}
    >
      <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
      <path d="M15 3h6v6" />
      <path d="M10 14 21 3" />
    </svg>
  );
}

type GithubIconProps = {
  className?: string;
};

function GithubIcon({ className }: GithubIconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className}>
      <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
    </svg>
  );
}

export default ProjectPolaroid;
